...
description: Implement a feature or fix by delegating the coding to opencode's headless CLI (opencode run, using whatever model the user has selected in opencode) while Claude Code acts as spec author and reviewer. Claude writes a detailed spec, dispatches opencode to implement, reviews opencode's diff against the project's own conventions, sends notes back into the SAME opencode session, and loops until every gate is green. Auto-approves ONLY after rigorous verification that the plan is completely fulfilled. Use when the user says "/opencode-implement <what they want>" or asks to have opencode implement something with Claude reviewing.
---

# opencode-implement — opencode implements, Claude Code reviews & approves

You are the **orchestrator and reviewer**. opencode's headless CLI (`opencode run`) is the **implementer**. The user gives you a request; you drive the whole loop and only report back when the work is genuinely done and verified.
...
- **The project's conventions apply even though opencode wrote the code** — you're accountable as if you wrote it.

## Step 0 — Learn THIS project's rules (do this first, once)

Learn the project's non-negotiables and reusable utilities from its convention docs (`CLAUDE.md` / `AGENTS.md` / `README`), and note its **check commands** (typecheck / lint / test — from `package.json` scripts, `Makefile`, or language-native tools) for the step-4 gates. opencode auto-loads `AGENTS.md` but won't necessarily read the other rule files itself, so put the task-relevant constraints into the spec (step 1). Keep the rules, reusables, and check commands handy — the spec, review, and gate steps refer back to them.

## The loop

**Fix-list, not a feature?** If the request is already concrete findings (`/code-review`, `/simplify`, a bug list), skip the PRD — pass them to opencode inline as the task (numbered), then run the same review → gates → loop. Write the full spec below only for open-ended features.

### 1. Author the spec (you, the reviewer)

The spec is the single biggest lever on how well opencode does. Every ambiguity you leave becomes a plausible-but-wrong guess and another review round. Invest here.
...
Write the spec as a PRD to `./tmp/opencode-implement/<feature-slug>.md`. **Never delete it — leave it for the user.** A file path beats a giant inline prompt.
...
### 2. Dispatch opencode (first round)

```bash
opencode run --title "<feature-slug>" \
  "Implement the spec in ./tmp/opencode-implement/<feature-slug>.md exactly. Follow every project constraint listed there. \
...
Capture opencode's printed output for context, but **do not trust it** — you review the real diff, not opencode's self-report.

Then get this run's session ID (the `--title` makes it findable) — every later round resumes exactly this ID:

```bash
opencode session list | grep "<feature-slug>"
```

### 3. Review (you)

Do a genuine senior review of what opencode actually did:

1. `git diff` — read every change. Cross-check touched files against the spec's **Files to touch**; anything extra is scope creep. **Undo only opencode's own mistakes, and only when you're sure they're not the user's** — preserve unrelated user changes. If you can't cleanly separate them, or the repo's left worse, stop and report with the diff summary instead of fixing it yourself.
2. **Replicate the project's guardrails by hand** — linters/formatters/hooks don't fire on opencode's edits. Check the diff obeys each mandatory rule (naming, forbidden APIs, no `eslint-disable` / `@ts-ignore` / `# noqa` unless the project sanctions it).
...
Produce a **numbered notes list**: each note is a concrete, actionable defect with the file:line and what's wrong. Rank by severity so opencode fixes the worst first, but **every note must be resolved before approval — minor and polish ones included, not just the critical ones.**
...
### 5. Send notes back into the SAME opencode session

If there are any notes at all, resume opencode's existing session so it keeps its context:
...
Cap at ~5 rounds. If opencode loops or regresses, STOP and report what's stuck and which files (with the diff summary) — don't burn rounds. The revert rule from step 3 applies: undo only opencode's mistakes, never the user's work.
...
- [ ] Every numbered acceptance criterion from the spec is implemented and you verified it in the actual diff (not from opencode's summary).
...
- **Rounds** — how many opencode iterations it took and the gist of what you sent back.
...
- Assumes `opencode` is installed and logged in — don't pre-check auth; just dispatch. Only if a dispatch returns an auth error, tell the user to run `opencode auth login` and stop.
- `--title <feature-slug>` + `opencode session list` + `-s <session-id>` pins every round to this run's own session, so the loop stays cheap (opencode keeps its context; you send only deltas) and several runs can go in parallel Claude sessions safely. Never use `-c` / `--continue` — it grabs the most recent session, which may belong to another run.
- If the user's request includes a model override (e.g. "use anthropic/claude-sonnet-5"), honor it via `-m provider/model` (plus `--variant` for reasoning effort where the provider supports it) but keep everything else identical.
- `--agent plan` (read-only) is available if you want opencode to propose a plan before writing on a risky/large task — optional, use judgment.
